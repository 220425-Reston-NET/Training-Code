...
Asynchronous asyncObject = new Asynchronous();

Console.WriteLine("==========Asynchronous==============");
Task<string> riceTask =  asyncObject.CookRice();
Task<string> meatTask =  asyncObject.CookMeat();
Task<string> veggiesTask =  asyncObject.CookVeggies();

//PUtting them in a list of tasks
List<Task<string>> dinnerTask = new List<Task<string>>(){riceTask, meatTask, veggiesTask};

while (dinnerTask.Count>0)
{
    //Will execute all the task and then wait for the first person to finish their task and put it in this variable
    Task<string> done = await Task.WhenAny(dinnerTask);
    Console.WriteLine(done.Result);

    dinnerTask.Remove(done);
}
